    ...
    const fetchEvents = async () => {
      try {
        ...
        if (eventsResponse.status === 403) {
          console.log("Cannot access events");
          setEvents([]);
          setIsLoading(false);
          return;
        }
          ...
          <List.Item>
            ...
            >
              {event.photos && event.photos.length > 0 && (
                <div style={{ position: 'relative', width: '100%', height: 200 }}> {/* Define a fixed height */}
                  <Image
                    src={event.photos[0].photo}
                    alt={`Thumbnail for Event ID ${event.event_id}`}
                    layout="fill" // This makes the image cover the defined area
                    objectFit="cover" // This ensures the image covers the div without distorting aspect ratio
                  />
                </div>
              )}
